...
def process_case(case_index):
    ...
    time = []
    for time_index in range(len(restart)):
        intehead = restart["INTEHEAD", time_index]
    ...
    ny = int(intehead0[9])
    nz = int(intehead0[10])
    saltp = np.array(restart["SGAS", report_step], dtype=np.float32) * 0.0
    ...
    indicator_mass = np.max(indicator_mass, axis=1)
...
for i, schedule in enumerate(schedules):
    ...
    with open(f"{whr}/co2_{i}.toml", "w", encoding="utf8") as file:
        file.write(filledtemplate)

for i in range(mt.floor(nsimulations / NPRUNS)):
    processes = []
    for j in range(NPRUNS):
        case_index = NPRUNS * i + j
        processes.append(
            subprocess.Popen(
                [
                    "pyopmnearwell",
