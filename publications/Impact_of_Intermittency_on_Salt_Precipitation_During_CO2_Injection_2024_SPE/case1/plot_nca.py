...
def nca(r_d, m_r):
    ...
    return (M_U * m_r * L_C) / (RHOG * PHI * PERM * P_E * HEIGHT * 2 * 3.1416 * r_d)
...
for i, qrate in enumerate(QRATE):
    ...
    n_ca = nca(r, qrate)
    axis.plot(r, n_ca, lw=3, color=COLORS[i], label=f"M = {qrate} kg/s")
axis.set_xlabel(r"d [m]")
axis.set_ylabel(r"n$_{ca}$ [-]")
axis.set_yscale("log")
axis.legend()
fig.savefig("nca.png", bbox_inches="tight")
