...
for i in range(mt.floor(len(GRATES) / NPRUNS)):
    ...
    for j in range(NPRUNS):
        if DELETE == 1:
            command += f"pyopmnearwell -i {EOR}_{NPRUNS*i+j}.toml -o {EOR}_{NPRUNS*i+j} -m single -v 0 & "
        else:
            command += f"pyopmnearwell -i {EOR}_{NPRUNS*i+j}.toml -o {EOR}_{NPRUNS*i+j} -m single & "
    command += "wait"
    os.system(command)
    for j in range(NPRUNS):
        smspec = Summary(f"./{EOR}_{NPRUNS*i+j}/{EOR.upper()}_{NPRUNS*i+j}.SMSPEC")
...
for i in range(remaining):
    if DELETE == 1:
        command += f"pyopmnearwell -i {EOR}_{finished+i}.toml -o {EOR}_{finished+i} -m single -v 0 & "
    else:
        command += f"pyopmnearwell -i {EOR}_{finished+i}.toml -o {EOR}_{finished+i} -m single & "
command += "wait"
os.system(command)
for i in range(remaining):
    smspec = Summary(f"./{EOR}_{finished+i}/{EOR.upper()}_{finished+i}.SMSPEC")
    names["wat_inj_vol"].append(smspec["FWIT"].values[-1])
    names["wat_pro_vol"].append(smspec["FWPT"].values[-1])
    names["gas_inj_vol"].append(smspec["FGIT"].values[-1])
    names["gas_pro_vol"].append(smspec["FGPT"].values[-1])
