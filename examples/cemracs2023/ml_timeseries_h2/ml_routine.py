...
y0 = scale_y0.transform(y0_all[x_all < TINJ+TSTOP+TOPER].reshape(-1, 1)).squeeze() # Keep only the data before the assessment
y1 = scale_y1.transform(y1_all[x_all < TINJ+TSTOP+TOPER].reshape(-1, 1)).squeeze() # Keep only the data before the assessment
# split into train and test sets
train_size = math.floor(len(x0) * .7)
...
model0 = Sequential()
model0.add(Dense(12, input_shape=(look_back,), activation='relu'))
...
model1 = Sequential()
model1.add(Dense(12, input_shape=(look_back,), activation='relu'))
