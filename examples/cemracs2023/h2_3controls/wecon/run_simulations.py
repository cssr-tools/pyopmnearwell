...
nsimulations = 3**NSCHED #The number of inj/prod combinations given the number of changues in the injection (NSCHED)
schedules = [seq for seq in itertools.product("012", repeat=NSCHED)]
seq_string = ["".join(seq) for seq in itertools.product("012", repeat=NSCHED)]
for i, row in enumerate(schedules):
    schedules[i] = [int(column) for column in row]
ordered = np.argsort(np.array([sum(row) for row in schedules]) + np.array([sum([1.5*(ent>1) for ent in row]) for row in schedules]))
...
np.save('1-fgpt_fgit', (fgit-fgpt) / fgit)
