...
TPERIOD = 120 # Duration of one period in days
...
for i in range(mt.floor(nsimulations / NPRUNS)):
    ...
    for j in range(NPRUNS):
        ...
        smspec = EclSum(f"./co2_{NPRUNS*i+j}/output/RESERVOIR.SMSPEC")
        unrst = EclFile(f"./co2_{NPRUNS*i+j}/output/RESERVOIR.UNRST")
        init = EclFile(f"./co2_{NPRUNS*i+j}/output/RESERVOIR.INIT")
        grid = EclGrid(f"./co2_{NPRUNS*i+j}/output/RESERVOIR.EGRID")
...
for j in range(remaining):
    ...
    z_centers = 0.5 * (zcord[1:] + zcord[:-1])
    nz_cells = len(z_centers)
    smspec = EclSum(f"./co2_{finished+j}/output/RESERVOIR.SMSPEC")
    unrst = EclFile(f"./co2_{finished+j}/output/RESERVOIR.UNRST")
    init = EclFile(f"./co2_{finished+j}/output/RESERVOIR.INIT")
    grid = EclGrid(f"./co2_{finished+j}/output/RESERVOIR.EGRID")
    phiv = np.array(init.iget_kw("PORV")[0][:])
    sgas = np.array(unrst.iget_kw("SGAS")[-1][:])
    r_s = np.array(unrst.iget_kw("RSW")[-1][:])
    rhog = np.array(unrst.iget_kw("GAS_DEN")[-1][:])
