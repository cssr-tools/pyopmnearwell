...
for i in [0, 5, 9]:
    ...
    bhp_data: np.ndarray = injection_rate_per_second_per_cell_member / target + pressure
    bhp_analytical: np.ndarray = (
        injection_rate_per_second_per_cell_member / np.array(WI_analytical) + pressure
    )
    plt.figure()
    plt.scatter(
        timesteps,
