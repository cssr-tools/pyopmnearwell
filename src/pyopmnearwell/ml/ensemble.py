""""Run high-fidelity nearwell simulations in OPM-Flow for an ensemble of varying inpout
Args.
...
def calculate_radii(gridfile: str) -> np.ndarray:
...
def calculate_WI(
    ...
    for i, datapoint in enumerate(pressure_data):
        p_w = datapoint[..., 0][..., None]  # Bottom hole pressure extracted at the first cell.
...
# def plot_WI_2D(features: data: np.ndarray, runspecs: dict[str, Any]) -> None:
