...
;
                  ...
                  ) const 
    {
            ...
            // Inside the domain
                ...
                for (int j = 0; j < num_cell_features; ++j) {
                    ...
                    // a bug?
                        ...
                        // Take log.
                        features[i][j] = Value(log10(connection.Kh() / connection.connectionLength()));
                        }
        ...
        std::cout << "WI: " << pow10(unscaleFunction(out.data_[0],${ymin}, ${ymax}, target_min, target_max)) << std::endl;
        std::cout << "WI_scaled: " << getValue(out.data_[0]) << std::endl;

        // Undo log before returning
        return pow10(unscaleFunction(out.data_[0],${ymin}, ${ymax}, target_min, target_max));
    }
