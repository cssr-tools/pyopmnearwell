...
'RATE' ${f"{abs(dic['inj'][j][4]) / 0.0850397 : E}"}  1* 400/
% else:
'INJ0' 'OIL' ${'OPEN' if dic['inj'][j][4] > 0 else 'STOP'}
'RATE' ${f"{abs(dic['inj'][j][4]) / 998.108 : E}"}  1* 400/ 998.108
%endif
/
WCONPROD
'PRO0' ${'OPEN' if dic['inj'][j][4] < 0 else 'STOP'} 'GRAT' 2* ${f"{abs(dic['inj'][j][4]) / 0.0850397 : E}"} 2* ${dic['inj'][j][5] if dic['inj'][j][4] < 0 else ''}/
/
% if dic['econ']>0:
WECON
