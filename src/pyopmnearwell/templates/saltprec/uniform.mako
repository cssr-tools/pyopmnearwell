...
% for i in range(dic['nocells'][2]):
% if dic['inj'][j][3]>0:
INJ${i} GAS ${'OPEN' if dic['inj'][j][4]>0 else 'SHUT'} RATE ${f"{dic['inj'][j][4] / (dic['nocells'][2]*1.86843):E}"} 1* 480 /
% else:
INJ${i} WATER ${'OPEN' if dic['inj'][j][4]>0 else 'SHUT'} RATE ${f"{dic['inj'][j][4] / (dic['nocells'][2]*998.108):E}"} 1* 480 /
% endif
% endfor
/
% if dic["pvmult"]==0 or dic['grid']=='core': 
