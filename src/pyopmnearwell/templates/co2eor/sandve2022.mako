...
-- The files have been converted to templates to allow for grid refinement and simulating
...
${dic['nocells'][0]*dic['nocells'][0]*dic['rock'][i][4]}*${dic['rock'][i][3]/(dic['rock'][i][4])} ${'/' if loop.last else ''}\
% endfor
...
PORO
% for i in range(dic['satnum']):
${dic['nocells'][0]*dic['nocells'][0]*dic['rock'][i][4]}*${dic['rock'][i][2]} ${'/' if loop.last else ''}\
% endfor


PERMX
% for i in range(dic['satnum']):
${dic['nocells'][0]*dic['nocells'][0]*dic['rock'][i][4]}*${dic['rock'][i][0]} ${'/' if loop.last else ''}\
% endfor
...
PERMZ
% for i in range(dic['satnum']):
${dic['nocells'][0]*dic['nocells'][0]*dic['rock'][i][4]}*${dic['rock'][i][1]} ${'/' if loop.last else ''}\
% endfor
...
% if dic['write']==1:
RPTRST
BASIC=2 DENG DENO DENW BO BG BW KRG KRO KRW VGAS VOIL VWAT /
% endif
...
/
WCONPROD
PROD OPEN ORAT 12000 4* ${dic["probhp"]} /
/
