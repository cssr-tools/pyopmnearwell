...
WELSPECS
% if dic['inj'][0][3] > 0:
'INJ0'  'G1'  ${max(1, 1+mt.floor(dic['noCells'][1]/2))} ${max(1, 1+mt.floor(dic['noCells'][1]/2))} 1*  'GAS' /
%else:
'INJ0'  'G1'  ${max(1, 1+mt.floor(dic['noCells'][1]/2))} ${max(1, 1+mt.floor(dic['noCells'][1]/2))} 1*  'WATER' /
%endif
% if dic["pvMult"] == 0:
% if dic['grid'] != 'cartesian':
